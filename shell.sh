...
sleep 1
if [ $(aws s3 ls "s3://elklogs-${accountId}" | grep 'NoSuchBucket' &> /dev/null) == 0 ] 
then
...
sleep 2
`aws s3 cp --quite --ignore-glacier-warnings --only-show-errors functionbeat/securitygroup.json s3://elklogs-${accountId}/securitygroup.json`
sleep 2
...
`zip -r functionbeat/package-aws.zip functionbeat/functionbeat.yml`
sleep 2
...
sleep 2
#`aws s3 cp functionbeat/package-aws.zip s3://elklogs-${accountId}/package-aws.zip`